...
var rangeStart = 25;
var rangeStop = 46;
var resultat = printRange(rangeStart, rangeStop)
 ...
 */
 ...
 var rangeStart = 46;
 var rangeStop = 25;
 var resultat = printRangeReversed(rangeStart, rangeStop)
 ...
 */
 ...
 var rangeStart = 25;
 var rangeStop = 46;
 var resultat = printAnyRange(rangeStart, rangeStop)
 ...
 function printAnyRange(rangeStart, rangeStop){
    ...
    if(rangeStart < rangeStop){
        lista = printRange(rangeStart, rangeStop);
        function printRange(rangeStart, rangeStop){
    ...
    if(rangeStart > rangeStop){
        lista = printRangeReversed(rangeStart, rangeStop);
        function printRangeReversed(rangeStart, rangeStop){
...
var ord = 'grey';
var siffra = 12;
var svar = stringRepeat(ord, siffra);

function stringRepeat(ord, siffra){
...
var rangeStop2 = 547;
var value = 434;
var svar = inRange(rangeStart2, rangeStop2, value);

function inRange(rangeStart2, rangeStop2, value){

    var svar = Boolean (value > rangeStart2 && value < rangeStop2);
...
var degrees = 32;
var svar = degreesToRadians(degrees);

function degreesToRadians(degrees){
    var svar = degrees / 180 * 3.14159265;
    svar = svar.toFixed(4)
 ...
 *
 * Create a function called `fizzfBuzz()` that takes two arguments `start` and
...
var start = 1;
var stop = 30;
var svar = fizzfBuzz(start, stop);

function fizzfBuzz(start, stop){
    var lista = '';
    while (start <= stop){
 ...
 * The dealers hand should be three cards with the values: `3, 6, 11`.
...
var dealerCard2 = 6;
var dealerCard3 = 11;
var resultat = printSum(playerCard1, playerCard2, playerCard3, dealerCard1, dealerCard2, dealerCard3);


function printSum(playerCard1, playerCard2, playerCard3, dealerCard1, dealerCard2, dealerCard3){
    var playerSum = 'Player: ' + (playerCard1 + playerCard2 + playerCard3) + ', ';
    var dealerSum = 'Dealer: ' + (dealerCard1 + dealerCard2 + dealerCard3);
    var resultat = playerSum + dealerSum;
    return resultat;
}
 ...
 */
 ...
 var dealerCard2 = 6;
 var dealerCard3 = 11;
 var resultat = printResult(playerCard1, playerCard2, playerCard3, dealerCard1, dealerCard2, dealerCard3);
 
 
 function printResult(playerCard1, playerCard2, playerCard3, dealerCard1, dealerCard2, dealerCard3){
     var playerResult = (playerCard1 + playerCard2 + playerCard3);
     var dealerResult = (dealerCard1 + dealerCard2 + dealerCard3);

        if(playerResult < 21){
            playerResult = 'Player: safe, ';
        }
        else if(playerResult == 21){
            playerResult = 'Player: black jack, ';
        }
        else{
            playerResult = 'Player: busted, ' 
        }

        if(dealerResult < 21 && dealerResult <= 16){
            dealerResult = 'Dealer: safe';
        }
        else if(dealerResult >=17 && dealerResult < 21){
            dealerResult = 'Dealer: stop';
        }
        else if(dealerResult == 21){
            dealerResult = 'Dealer: black jack';
        }
        else{
            dealerResult = 'Dealer: busted';
        }

        var resultat = playerResult + dealerResult;
...
var resultat = calculateInterest(startMoney, years, interest);

function calculateInterest (startMoney, years, interest){
    
    for(let i = 0; i < years; i++){
        startMoney = startMoney * interest + startMoney;
    }
